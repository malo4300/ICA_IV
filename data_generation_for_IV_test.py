...
def sim(mode, B, iter):
    ...
    for i in tqdm.tqdm(range(B)):
        ...
        pd.DataFrame(data.mixing_matrix_observed).to_csv(f"sim_data_VarEM/mixing_matrix/true_mixing_{i}.csv", index = False)
        ...
        pd.DataFrame(data.signals).to_csv(f"sim_data_VarEM/signals/true_signals_{i}.csv", index = False)
        pd.DataFrame(data.data_observed).to_csv(f"sim_data_VarEM/data/data_obs_{i}.csv", index = False)
