...
def sim(mode, B, iter, dgp_, name):
    ...
    for i in tqdm.tqdm(range(B)):
        data.generate_data(n,I, J, random_state=i, init_range = [.5,1])
        ...
        pd.DataFrame(est.Signals).to_csv(f"extended_dgp/bounded_coef/signals/estimated_signals_CausalVarEMVarEM_{name}_{i}.csv", index = False)
...
if __name__ == "__main__":
    ...
    I = J-1
    n = 1000
    sim("VarEM", 100, 100, dgp_extended, "init05_1")