...
p_values_indp = read.csv("ind_tests/increase_conf/LDAG_VarEM_init13_old.csv", header =1)[-1]
...
for (i in 1:100) {
  ...
  sg_path = get_path("extended_dgp/bounded_coef/signals/estimated_signals_VarEM_init13_", seed)
  #dt_path = get_path_seed("increase_conf/data/data_obs_conf_", seed,conf)data_obs_init13_
  dt_path = get_path("extended_dgp/bounded_coef/data/data_obs_init13_", seed)
...
#saveRDS(cand, file="IV_test_results/increased_confounding/LDAG_CausalVarEM_init13_old.RData")
cand = readRDS(file="IV_test_results/increased_confounding/LDAG_CausalVarEM_conf_1.RData")
...
for (i in 1:l){
  ...
  #signals = read.csv(get_path("extended_dgp/bounded_coef/signals/estimated_signals_CausalVarEM_init13_", seed))
  ...
  #data = read.csv(get_path("extended_dgp/bounded_coef/data/data_obs_init13_", seed))
  confounder_source_col = as.numeric(cand[indx])
  df = data.frame(y = data[,I], treatment = data[, treatment_col] , signals = signals[,1:6])
  fit = lm(y~.-1,df)  
  estimated_treatment_efect[i] = coef(fit)["treatment"] 
  true_treatment_effect[i] = get_true_treatment_from_mixing_matrix(path = paste("increase_conf/mixing_matrix/true_mixing_large_conf_1_", conf ,"_",sep= ""),i = seed)
  #true_treatment_effect[i] = get_true_treatment_from_mixing_matrix( path = "extended_dgp/bounded_coef/mixing_matrix/true_mixing_init13_", i = seed)
}  
...
for (i in 1:l) {
  ...
  mm = read.csv(get_path("extended_dgp/bounded_coef/mixing_matrix/estimated_mixing_CausalVarEM_init13_", seed))
...
points(true_treatment_effect,estimated_treatment_efect_ ,xlab = "True treatment", ylab = "Estimated treatment via OLS", col = "blue")
abline(a = 0, b = 1)

legend("topleft", legend = c("Column Extraction", "OLS Estimate with estimated confounder column", "OLS only using sources 1:5"), 
       col = c("red", "black", "blue"), pch = c(1, 1,1))
